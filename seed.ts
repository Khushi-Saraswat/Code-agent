...
// // Add this debug line right after loading .env
  ...
  try {
    ...
    try {
      ...
      if (existingCategories.length > 0) {
        console.log(
          `Found ${existingCategories.length} existing categories, deleting...`
        );
        await Promise.all(
      ...
      console.log(
        "No existing categories found or error clearing them:",
        error
      );
    }
    ...
    // Create categories sequentially to avoid transaction conflicts
    for (const category of categories) {
      try {
        const parentCategory = await payload.create({
          collection: "categories",
        ...
        if (category.subcategories?.length) {
          // Create subcategories sequentially
          for (const subCategory of category.subcategories) {
            await payload.create({
              collection: "categories",
              data: {
